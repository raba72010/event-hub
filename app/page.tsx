...
import { Menu, User, Calendar, Clock, MapPin, ArrowRight, Star, X } from "lucide-react"
...
export default function ProWebinarHub() {
  ...
  const [isLoading, setIsLoading] = useState(true) // <--- Variable defined here as 'isLoading'
    ...
    async function fetchEvents() {
      try {
        ...
        const mappedEvents: Event[] = (data || []).map((row: any) => {
          ...
          const status = row.status === "past" ? "on-demand" : "upcoming"
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
          ...
          <div className="hidden md:flex items-center gap-4">
            {isSignedIn ? (
              <>
                ...
                <Button onClick={handleLogout} variant="outline" size="sm" className="border-slate-300">
                  Sign Out
                </Button>
              </>
            ) : (
      ...
      <section className="relative overflow-hidden bg-slate-900 py-20 md:py-32">
        ...
        <div className="container relative mx-auto px-4 text-center md:px-6">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-lg text-slate-400">
            Join expert-led sessions on Design, Coding, and Product. Level up your career with community-driven learning.
          </p>
          <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <Button size="lg" className="h-12 px-8 bg-white text-slate-900 hover:bg-slate-100 font-semibold rounded-full">
              Explore Events
            </Button>
            {isSignedIn && (
              <Link href="/admin">
      ...
      <section className="container mx-auto px-4 py-20 md:px-6">
        ...
        {/* 👇 FIXED: Used 'isLoading' instead of 'loading' */}
        {isLoading ? (
            ...
            {events.map((event) => (
              ...
              >
                ...
                <div className="relative h-48 w-full bg-slate-100 overflow-hidden">
                   ...
                   <div className="absolute top-4 right-4 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-slate-900 shadow-sm backdrop-blur-sm">
                     {event.category}
                   </div>
                </div>
                  ...
                  <div className="flex items-center gap-3 border-t border-slate-100 pt-4 mb-5">
                    ...
                    <div className="text-sm">
                      <p className="font-medium text-slate-900">
                        {event.speakers?.[0]?.name || event.speakers?.[0]?.name || "Guest Speaker"}
                      </p>
                      <p className="text-xs text-slate-500">
                        {event.speakers?.[0]?.company || "Presenter"}
                      </p>
                    </div>
                  </div>
        ...
        {!isLoading && events.length === 0 && (
          <div className="text-center py-20 bg-slate-50 rounded-2xl border border-dashed border-slate-300">
            ...
            <p className="text-slate-500 mt-1">Check back later or login to Admin to create one.</p>
          </div>
        )}
      </section>
